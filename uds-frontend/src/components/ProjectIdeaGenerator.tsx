...
export const ProjectIdeaGenerator = () => {
  ...
  const viewGuidance = async (idea: Idea) => {
    ...
    toast({ title: "Guidance ready", description: "Simple step-by-step plan (frontend fallback)." });
  };
    ...
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {inventoryOptions.map((it) => (
              <label
                key={it.id}
                className={`p-2 border rounded cursor-pointer ${selectedIds.includes(it.id) ? "bg-primary/10 border-primary" : "bg-card"}`}
              >
        ...
        <div>
          ...
          <select value={skill} onChange={(e) => setSkill(e.target.value as Idea["skillLevel"])} className="w-full p-2 border rounded bg-card text-foreground">
            <option value="beginner">Beginner</option>
            <option value="intermediate">Intermediate</option>
            <option value="advanced">Advanced</option>
          </select>
          <div className="mt-4">
            <button disabled={loading} onClick={generateIdeas} className="px-4 py-2 bg-primary text-primary-foreground rounded disabled:opacity-50">
            ...
            <div key={idea.id} className="p-3 border rounded bg-card">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => viewGuidance(idea)} className="text-sm text-muted-foreground">View Guidance</button>
                </div>
              </div>
